...
const ShopProfile: React.FC<ShopProfileProps> = ({ shopId }) => {
  ...
  return (
    <section className="w-full flex flex-col items-center">
      <div className="w-full flex items-center justify-center h-[40vh] lg:min-h-[65vh] lg:py-4 md:py-4 py-0">
        <div
          className="w-full h-full lg:min-h-[50vh] flex flex-col justify-end max-w-[1300px] p-4 lg:rounded-lg md:rounded-lg rounded-none"
          style={{
            backgroundImage: shop?.shop_images?.[0]
              ? `url(${shop.shop_images[0].image_url})`
              : "none",
        ...
        >
          <div className="lg:min-w-[18rem] lg:max-w-[25rem] md:min-w-[17rem] min-h-[5rem] rounded-lg border bg-white border-gray-300 mb-5 p-6">
            <div className="w-full flex flex-row gap-4">
              <div className="h-[4rem] w-[7rem] rounded-md border relative">
                {loading ? (
                  ...
                  <Image
                    layout="fill"
                    alt="shop-image"
                    src={shop?.logo || "/images/placeholder.jpg"}
                    objectFit="contain"
                  />
                )}
              </div>
              <div className="flex flex-col gap-1">
                ...
                {/* <span className="text-muted-foreground text-sm">
                  <strong>{shop?.products.length}</strong> Total Products
                </span> */}
